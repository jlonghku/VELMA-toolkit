...
3. **`class_method='auto-weight'`**  
   ...
   - each iteration compares global class percentage of the resampled map with
     the original map (before coarsening);
   - update class weights to shrink the difference, clipped to a reasonable
     range.  
   Use this when you don’t know which class to weight, you just want “keep the
   histogram”.

4. **`class_method='auto-reassign'`**  
   - first do a plain per-block assignment but record top-k candidates in each
     block;  
   - then look at the global histogram, and selectively reassign blocks that
     have the target class as 2nd/3rd candidate;  
   - reassign only a small number to hit the target percentages.  
   Use this when class distribution is critical (e.g. scenario modeling).
